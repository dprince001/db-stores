...
const MensRoute = () => {
  const {handleFilterCategory, handleFilterPrice} = useContext(FilterContext);
  const {data, isPending, error} = useFetch(`https://dprince001.github.io/db.json`);

  const displayProducts = handleFilterCategory(data, 'mens');
  ...
  return (
    <div>
      <div className='w-[85%] mt-[20px] m-auto min-h-screen'>
        ...
        {!isPending && <FilterBarComp totalItems={displayProducts.length}/>}
        {
